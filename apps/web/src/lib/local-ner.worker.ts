...
async function initialize(modelId: string, revision: string) {
  try {
    ...
    classifier = await pipeline("token-classification", modelId, { device: supportsWebGpu ? "webgpu" : "wasm", dtype: "q8", revision }) as unknown as typeof classifier;
    self.postMessage({ type: "ready", modelId, revision, device: supportsWebGpu ? "webgpu" : "wasm" });
  } catch (error) {
